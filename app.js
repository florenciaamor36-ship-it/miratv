...
function importarTexto(){const texto=document.getElementById('m3uTexto').value.trim();const status=document.getElementById('importTextStatus');if(!texto){status.textContent='⚠️ Pegá el texto primero';return}if(!texto.includes('#EXTINF')){status.textContent='❌ No parece una lista M3U válida';return}const p=parseM3U(texto);if(!p.length){status.textContent='❌ No se encontraron canales';return}canales=p;renderTV();status.textContent=`✅ ${p.length} canales importados`;document.getElementById('m3uTexto').value='';switchPage('home')}
