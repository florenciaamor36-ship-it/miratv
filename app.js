...
function renderTV() {
    ...
    const cats = {};
    canales.forEach(c => { const cat = c.group || 'General'; if (!cats[cat]) cats[cat] = []; cats[cat].push(c); });
    container.innerHTML = '';
    Object.entries(cats).forEach(([cat, items]) => {
        const sec = document.createElement('div');
        sec.innerHTML = `<div class="section-header open" onclick="this.classList.toggle('open');this.nextElementSibling.classList.toggle('open')"><h3>${cat} (${items.length})</h3><span class="arrow">▶</span></div><div class="section-content open"><div class="grid"></div></div>`;
        const grid = sec.querySelector('.grid');
        items.forEach(c => grid.appendChild(card(c, 'tv')));
...
async function importarLista() {
    ...
    try {
        ...
        if (!text) { status.textContent = '❌ No se pudo descargar'; hideCounter(); return; }
        const p = parseM3U(text);
        if (!p.length) { status.textContent = '❌ No se encontraron canales'; hideCounter(); return; }
        const nombre = url.split('/').pop()?.split('?')[0] || 'listaImportada';
        p.forEach(c => { c.cat = nombre; if (!c.group) c.group = nombre; });
        canales = p;
        renderTV();
        hideCounter();
        status.textContent = `✅ ${p.length} canales importados`;
        document.getElementById('m3uUrl').value = '';
        switchPage('home');
    } catch (e) { status.textContent = '❌ Error al descargar'; hideCounter(); }
}
