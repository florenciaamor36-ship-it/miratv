...
function renderTV() {
    ...
    container.innerHTML = '';
    const sections = [
        ['📺 Argentina', g('Argentina'), 'tv'],
        ['⚽ Deportes', g('Deportes'), 'sports'],
        ['📰 Noticias', g('Noticias'), 'tv'],
        ['🎵 Música', g('Música'), 'tv'],
        ['👶 Infantil', g('Infantil'), 'kids'],
        ['📚 Documentales', g('Documentales'), 'tv'],
        ['🎬 Películas', g('Películas'), 'movie'],
    ];
    sections.forEach(([t, items, tp]) => {
        if (items.length) container.appendChild(makeSection(t, items.length, items, tp));
    });
...
function openPlayer(c) {
    playerCh = c;
    document.getElementById('playerName').textContent = c.name || 'Canal';
